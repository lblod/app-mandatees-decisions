...
function transformTriples(fetch, triples) {
  ...
  Y_ATTEMPTS, SLEEP_TIME_AFTER_FAILED_REASONING_OPERATION);
}
...
function mainConversion(fetch, triples) {
  ...
  return fetch("http://reasoner/reason/example", requestOptions)
...
async function transformLandingZoneGraph(fetch, constants) {
  ...
  const response = await fetch(`http://reasoner/reason/op2dl/main?data=${encodeURIComponent(`${LANDING_ZONE_DATABASE_ENDPOINT}?default-graph-uri=&query=CONSTRUCT+%7B%0D%0A%3Fs+%3Fp+%3Fo%0D%0A%7D+WHERE+%7B%0D%0A+GRAPH+<${LANDING_ZONE_GRAPH}>+%7B%0D%0A%3Fs+%3Fp+%3Fo%0D%0A%7D%0D%0A%7D&should-sponge=&format=text%2Fplain&timeout=0&run=+Run+Query`)}`);
